...
void hal_led_start_pinball(void){
	#define DIR_INPUT 0
	#define DIR_OUTPUT 1
	ioport_set_pin_dir( IOPORT_CREATE_PIN(PIOB, 1), DIR_OUTPUT ); //left light
	ioport_set_pin_dir( IOPORT_CREATE_PIN(PIOA, 23), DIR_OUTPUT );//right light
	ioport_set_pin_dir( IOPORT_CREATE_PIN(PIOC, 25), DIR_OUTPUT );//game over
}
...
void hal_led_write_pinball(tLedNum lednum, tLedState state){
	uint32_t IOLine = 0;
	switch(lednum){
		case Led0: IOLine = IOPORT_CREATE_PIN(PIOB, 1); break; //left
		case Led1: IOLine = IOPORT_CREATE_PIN(PIOA, 23); break; //right
		case Led2: IOLine = IOPORT_CREATE_PIN(PIOC, 25); break; //game over
		default: /* Error */ ;
	}
}
...
tLedState hal_led_read_pinball(tLedNum lednum){
	uint32_t IOLine = 0;
...
void hal_adc_pinball_configure(void){
...
uint32_t hal_adc_pinball_read(uint32_t Other_Sensor){
		volatile uint32_t IR_Sensor;
	switch(Other_Sensor){
		case 2: IR_Sensor = adc_get_channel_value(ADC, ADC_CHANNEL_2); break; //left
		case 3: IR_Sensor = adc_get_channel_value(ADC, ADC_CHANNEL_3); break; //right
		case 4: IR_Sensor = adc_get_channel_value(ADC, ADC_CHANNEL_4); break; //game over
		default: /* Error */ ;
	}
return IR_Sensor;
}
...
void hal_display_putc_pinball(const char *string){
