...
enum tLedStatePinball {LightOff=false, LightOn};								/**< enumeration for LED states */
enum tLedNum	{ Led0 = 0, Led1, Led2, Led3, Led4 };					/**< enumeration for LEDs in the board */
enum tLedNumPinball {LedPointsLeft = 0, LedPointsRight, LedGameOver};
...
enum tMemRegionId { MemRegSystem = 0, MemRegApp };			/**< enumeration for memory regions */	
...
void hal_led_write_pinball(tLedNum, tLedState);
tLedState hal_led_read_pinball(tLedNum);

void hal_adc_pinball_configure(void);
uint32_t hal_adc_pinball_read(uint32_t);
